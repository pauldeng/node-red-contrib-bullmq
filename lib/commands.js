...
const { getLegacySchedulerId, normalizeAddRequest, serializeScheduler } = require("./scheduler");
...
async function dispatchAdd(queue, msg) {
  ...
  return serializeJob(await queue.add(request.name, request.data, request.opts));
}
...
async function dispatchCommand(queue, msg = {}) {
  ...
  switch (cmd) {
    ...
    case "getJobScheduler":
      return serializeScheduler(await queue.getJobScheduler(getLegacySchedulerId(msg)));
