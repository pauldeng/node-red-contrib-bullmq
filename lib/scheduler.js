...
function normalizeRepeatOptions(repeat) {
  ...
  if (normalized.cron && normalized.pattern && normalized.cron !== normalized.pattern) {
    throw new Error("repeat.cron and repeat.pattern must match when both are supplied");
  }
  if (normalized.cron) {
...
function getLegacySchedulerId(msg = {}) {
  const schedulerId = String(msg.schedulerId || msg.jobid || msg.jobId || "").trim();
  if (!schedulerId) {
